...
export const ACTIVE_PILLARS = Object.freeze(['team-form','h2h','attack-v-defence']);
export const PILLAR_ADJUSTMENTS = Object.freeze({ teamFormMax: 0.15, h2hMax: 0.075, combinedMax: 0.20 });
...
export function calculateThreePillarPrediction({
  homeForm, awayForm, homeH2H, awayH2H, attackDefence
} = {}) {
  const hf=Number(homeForm), af=Number(awayForm), hh=Number(homeH2H), ah=Number(awayH2H);
  if (![hf,af,hh,ah].every(Number.isFinite) || !attackDefence) return null;

  const formSignal = clamp((hf-af)/100 * PILLAR_ADJUSTMENTS.teamFormMax,
    -PILLAR_ADJUSTMENTS.teamFormMax, PILLAR_ADJUSTMENTS.teamFormMax);
  const h2hSignal = clamp((hh-ah)/100 * PILLAR_ADJUSTMENTS.h2hMax,
    -PILLAR_ADJUSTMENTS.h2hMax, PILLAR_ADJUSTMENTS.h2hMax);
  const combined = clamp(formSignal+h2hSignal,
    -PILLAR_ADJUSTMENTS.combinedMax, PILLAR_ADJUSTMENTS.combinedMax);

  const homeLambda = clamp(attackDefence.homeBase*(1+combined),ENGINE.lambdaMin,ENGINE.lambdaMax);
  const awayLambda = clamp(attackDefence.awayBase*(1-combined),ENGINE.lambdaMin,ENGINE.lambdaMax);
  const poisson = calculatePoisson(homeLambda,awayLambda);
  const outcomes = [
    ['home',poisson.home],['draw',poisson.draw],['away',poisson.away]
  ].sort((a,b)=>b[1]-a[1]);

  return {
    baseXG:{home:Math.round(attackDefence.homeBase*100)/100,away:Math.round(attackDefence.awayBase*100)/100},
    formAdjustment:formSignal,
    h2hAdjustment:h2hSignal,
    combinedAdjustment:combined,
    finalXG:{home:Math.round(homeLambda*100)/100,away:Math.round(awayLambda*100)/100},
    probabilities:{home:poisson.home,draw:poisson.draw,away:poisson.away},
    predictedOutcome:outcomes[0][0],
