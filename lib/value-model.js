...
function valueRankScore({ modelProbability, edge, expectedValue, heat }) {
  const probabilityScore = clamp(
...
export function evaluatePick(pick) {
  ...
  if (pick?.modelAgreement === false) {
    reasons.push('Form/xG and the goal model disagree.');
  }
  ...
  if (pick?.modelAgreement !== true) footballNotes.push('Full model-agreement confirmation is pending.');
  ...
  if (!hasSelectionOdds || !market.available) {
    ...
    return {
      ...
      priceStatus: 'pending',
      footballScore,
      football: { modelProbability, heat, contradictions },
    };
  }
  ...
  if (coreFootballReady && corePriceReady) {
    return {
      ...
      footballScore,
      value: { ...value, rankScore: valueRankScore(value) },
    };
  }
...
export function selectCorePicks(picks) {
  ...
  const core = evaluated
    ...
    .slice(0, VALUE_RULES.maxCorePicks);
  ...
  const belowCut = evaluated
    .filter(({ assessment }) => assessment.grade === 'core')
    .map(({ pick, assessment }) => ({ ...pick, value: assessment.value, assessment }))
    .sort((a, b) => b.value.rankScore - a.value.rankScore)
    .slice(VALUE_RULES.maxCorePicks);
