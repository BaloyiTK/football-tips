...
export const PILLAR_WEIGHTS = Object.freeze({ teamForm: 0.40, h2h: 0.20, attackDefence: 0.40 });
...
export function calculateThreePillarRating({teamForm,h2h,attackDefence}={}) {
  const p1=Number(teamForm), p2=Number(h2h), p3=Number(attackDefence);
  if (![p1,p2,p3].every(Number.isFinite)) return null;
  return Math.round((p1*PILLAR_WEIGHTS.teamForm+p2*PILLAR_WEIGHTS.h2h+p3*PILLAR_WEIGHTS.attackDefence)*10)/10;
}
