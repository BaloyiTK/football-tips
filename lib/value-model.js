export const MODEL_VERSION = '1.9';
...
export const VALUE_RULES = Object.freeze({
  ...
  maxAwayNoVigWinProbability: 0.60,
  maxAwayScoreProbability: 0.35,
  minHomeCleanSheetProbability: 0.65,
  minAwayScoreAdvantageVsLeague: 0.10,
  maxPriceShorteningFromOpen: 0.10,
  maxSoftRiskFlags: 1,
...
export function isOneXPick(pick) {
  const covered = Array.isArray(pick?.coveredOutcomes)
    ? pick.coveredOutcomes.map(String).sort()
    : [];
  if (covered.length === 2 && covered[0] === '1' && covered[1] === 'X') return true;

  const market = String(pick?.market || '').toUpperCase().replace(/\s+/g, ' ');
  return market.includes('(1X)') || market.includes(' 1X') || market.endsWith('1X');
}
...
function normaliseNonOpposingSignal(value) {
  if (value === 'neutral') return true;
  return normaliseSignal(value);
}
...
export function validateFootballEvidence(pick) {
  ...
  const signals = {
    ...
    h2h: normaliseNonOpposingSignal(evidence.h2h),
    opponentStrength: normaliseSignal(evidence.opponentStrength),
    motivation: normaliseNonOpposingSignal(evidence.motivation),
    teamNews: normaliseNonOpposingSignal(evidence.teamNews),
    scheduleFatigue: normaliseNonOpposingSignal(evidence.scheduleFatigue),
    awayScoringThreat: normaliseSignal(evidence.awayScoringThreat),
    leagueBaseline: normaliseSignal(evidence.leagueBaseline),
    sampleBlend: normaliseSignal(evidence.sampleBlend),
    oppositionAdjusted: normaliseSignal(evidence.oppositionAdjusted),
    goalkeeperQuality: normaliseNonOpposingSignal(evidence.goalkeeperQuality),
    setPieceRisk: normaliseNonOpposingSignal(evidence.setPieceRisk),
    marketMove: normaliseNonOpposingSignal(evidence.marketMove),
    leagueBaseline: normaliseSignal(evidence.leagueBaseline),
    sampleBlend: normaliseSignal(evidence.sampleBlend),
    oppositionAdjusted: normaliseSignal(evidence.oppositionAdjusted),
    goalkeeperQuality: normaliseNonOpposingSignal(evidence.goalkeeperQuality),
...
export function evaluatePick(pick) {
  ...
  if (!isOneXPick(pick)) {
    return {
      grade: 'skip',
      qualifies: false,
      reasons: ['Model v1.9 only evaluates home-or-draw (1X) selections.'],
      priceStatus: 'not-assessed',
      marketPolicy: '1X-only',
    };
  }
  const footballNotes = [];
  const priceNotes = [];
  ...
  const evidence = pick?.footballEvidence || {};
  const awayScoreProbability = parseProbability(
    pick?.footballEvidence?.awayScoreProbability ?? pick?.awayScoreProbability
  );
  const homeCleanSheetProbability =
    awayScoreProbability === null ? null : 1 - awayScoreProbability;
  const leagueAwayScoreBaseline = parseProbability(evidence.leagueAwayScoreBaseline);
  const awayScoreAdvantageVsLeague =
    awayScoreProbability === null || leagueAwayScoreBaseline === null
      ? null
      : leagueAwayScoreBaseline - awayScoreProbability;

  const softRiskFields = ['gameScriptRisk', 'refereeDisciplineRisk', 'weatherPitchRisk', 'homeFalseFavouriteRisk'];
  const softRiskFlags = softRiskFields.filter((field) => normaliseSignal(evidence[field]) === false);
  const softRiskCount = softRiskFlags.length;

  const openingOdds = Number(pick?.openingOdds);
  const priceShorteningFromOpen =
    Number.isFinite(openingOdds) && openingOdds > 1 && Number.isFinite(odds) && odds > 1
      ? (openingOdds - odds) / openingOdds
      : null;

  const marketEntries = pick?.marketOdds && typeof pick.marketOdds === 'object'
    ? Object.entries(pick.marketOdds).map(([key, value]) => [String(key), Number(value)])
    : [];
  const marketOverround = marketEntries.length >= 3 && marketEntries.every(([, value]) => Number.isFinite(value) && value > 1)
    ? marketEntries.reduce((sum, [, value]) => sum + 1 / value, 0)
    : null;
  const awayOddsEntry = marketEntries.find(([key]) => key === '2');
  const awayNoVigProbability =
    marketOverround && awayOddsEntry
      ? (1 / awayOddsEntry[1]) / marketOverround
      : null;
  ...
  if (
    ...
    awayNoVigProbability >= VALUE_RULES.maxAwayNoVigWinProbability
  ) {
    reasons.push(
      `Away side is an overwhelming market favourite (${(awayNoVigProbability * 100).toFixed(1)}% no-vig win probability).`
    );
  }
  if (
    awayScoreProbability !== null &&
    awayScoreProbability > VALUE_RULES.maxAwayScoreProbability
  ) {
    reasons.push(`Away scoring probability is too high (${(awayScoreProbability * 100).toFixed(1)}%; maximum is 35%).`);
  }
  if (awayScoreAdvantageVsLeague !== null && awayScoreAdvantageVsLeague < VALUE_RULES.minAwayScoreAdvantageVsLeague) {
    reasons.push(`Away scoring threat is not sufficiently below the league baseline (${(awayScoreAdvantageVsLeague * 100).toFixed(1)}pp below; minimum is 10pp).`);
  }
  if (priceShorteningFromOpen !== null && priceShorteningFromOpen > VALUE_RULES.maxPriceShorteningFromOpen) {
    reasons.push(`1X price has shortened too far from opening (${(priceShorteningFromOpen * 100).toFixed(1)}%; maximum is 10%).`);
  }
  if (softRiskFlags.length > VALUE_RULES.maxSoftRiskFlags) {
    reasons.push(`Too many contextual soft-risk flags oppose the pick: ${softRiskFlags.join(', ')}.`);
  }
  if (
    awayScoreAdvantageVsLeague !== null &&
    awayScoreAdvantageVsLeague < VALUE_RULES.minAwayScoreAdvantageVsLeague
  ) {
    reasons.push(
      `Away scoring threat is not sufficiently below the league baseline (${(awayScoreAdvantageVsLeague * 100).toFixed(1)}pp below; minimum is 10pp).`
    );
  }
  if (
    priceShorteningFromOpen !== null &&
    priceShorteningFromOpen > VALUE_RULES.maxPriceShorteningFromOpen
  ) {
    reasons.push(
      `1X price has shortened too far from opening (${(priceShorteningFromOpen * 100).toFixed(1)}%; maximum is 10%).`
    );
  }
  if (softRiskCount > VALUE_RULES.maxSoftRiskFlags) {
    reasons.push(
      `Too many contextual soft-risk flags oppose the pick: ${softRiskFlags.join(', ')}.`
    );
  }
  ...
  if (!hasSelectionOdds || !market.available) {
    ...
    return {
      ...
      football: {
        ...
        heat,
        contradictions,
        validation: footballValidation,
        awayNoVigProbability,
        awayScoreProbability,
        homeCleanSheetProbability,
        leagueAwayScoreBaseline,
        awayScoreAdvantageVsLeague,
        priceShorteningFromOpen,
        softRiskFlags,
      },
    };
  }
